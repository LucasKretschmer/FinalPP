...
public class Portal extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        /*try
        {
            if (ArquivoConfiguracao.getInstance().getValor("cloud") != null && ArquivoConfiguracao.getInstance().getValor("cloud").equals("S"))
            {
                response.sendRedirect("http://www.tecnicon.com.br");
            }
        ...
        {
            response.sendRedirect("/Tecnicon/index.html");
        } else
        {
            ...
            //Abrir chat automatico - notificações
            ...
            if (abreChat != null && !abreChat.equals("") && abreChat.split("§").length > 2)
            {
                ...
                if ((abreChat + datahora).equals(ValidaParametro.getInstance().getChat(usuario)))
                {
                    ValidaParametro.getInstance().setChat(usuario, "");
                    response.sendRedirect("/Tecnicon/index.html");
                } else
                {
                    ValidaParametro.getInstance().setChat(usuario, abreChat + datahora);
                }
            } else
            {
                abreChat = "";
            ...
            //request.getSession(true).setMaxInactiveInterval(1 * 60);
            ...
            request.getSession(true).setAttribute("navegadorswing", navegadorswing);
            request.getSession(true).setAttribute("navegadordelphi", navegadordelphi);
...
//                        sessao = a.get(0);
            ...
            } else
            {
                try
                {
                    ...
                    {
                        out.println(Index.retornaIndex(dispositivo, navegadordelphi, tipoacesso, navegadorswing, "L", request, abreChat));
                    } else
                    {
                        out.println(Index.retornaIndex(dispositivo, navegadordelphi, tipoacesso, navegadorswing, "L", request));
                    }
                    out.close();
                } catch (ExcecaoTecnicon ex)
                {
            ...
            /*
             ...
             if (browser.equals("desktop")) {
             } else if (browser.equals("iphone")) {
             response.setContentType("text/html");
             PrintWriter out = response.getWriter();
             out = response.getWriter();
             out.println("<h1>IPHONE<h1>");
             out.close();
             }
